  ...
  Future<void> _updateRecipe() async {
    ...
    try {
      ...
      final updatedRecipe = widget.recipe.copyWith(
        ...
        rating: double.tryParse(_ratingController.text.trim()) ?? widget.recipe.rating,
        review: int.tryParse(_reviewsController.text.trim()) ?? widget.recipe.review,
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(recipeProvider.errorMessage ?? 'Failed to update recipe.'),
            backgroundColor: Colors.redAccent,
          ),
        );
      }
          ...
          child: Column(
            ...
            children: [
                    ...
                    child: Stack(
                      ...
                      children: [
                        if (_newRecipeImageFile != null)
                          ...
                          const Center(
                            child: Icon(Iconsax.image, size: 44, color: AppColors.primary),
                          ),
                        Positioned(
                          bottom: 8,
                          right: 8,
              ...
              DropdownButtonFormField<String>(
                initialValue: categories.any((c) => c.name == _selectedCategory) ? _selectedCategory : null,
                decoration: InputDecoration(
                  labelText: 'Category *',
                  filled: true,
                  fillColor: theme.cardColor,
                        ...
                        child: Container(
                          ...
                          child: entry.localImageFile != null
                              ...
                              : (entry.imageUrl.isNotEmpty
                                  ? ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: SafeNetworkImage(imageUrl: entry.imageUrl, fit: BoxFit.cover),
                                    )
                ...
                child: Row(
                  ...
                  children: [
                    const Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Published Status', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                        SizedBox(height: 2),
                        Text('Published recipes are visible to audience users', style: TextStyle(color: AppColors.textSecondary, fontSize: 11)),
                      ],
                    ),
                    Switch(
                      value: _isPublished,
